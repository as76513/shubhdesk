...
- `src/leadClient.ts` — the only place that talks to the Amplify Data client (`generateClient<Schema>()`). All reads/writes for `Lead`, `Note`, `StaffProfile`, `Counter` go through here. Notable behaviors:
  ...
  - `moveStage()` detects the sales→RM handoff (moving into `meeting` from a sales stage with an `rmUsername` given), reassigns `owner`, and writes a system-type `Note` in the same call. Takes optional `nameOf`/`stageLabelOf` resolvers (passed in from `App.tsx`) so the note text reads "Moved from New Lead to Meeting" / "Handed off to Anita by Amol" with real names — without them it falls back to raw Cognito usernames and stage ids. **Only affects notes written after this resolver was added**; existing note text with baked-in IDs can't be fixed retroactively since it's stored free text, not a lookup (unlike `Note.author`, see below).
...
- **Dealer role**: `App.tsx` branches to a dedicated, minimal render (`me.role === "dealer"`) that skips the entire Lead pipeline UI — no board, no tabs, no drawer — and shows only `TradesView`. Admins get the same `TradesView` component via an extra "Trades" tab alongside the pipeline views, plus a `Dealer` column in the raw trade list (`nameOf(t.owner)`) since admin sees everyone's trades mixed together. `TradesView` computes a same-day "Dealer Brokerage" summary (total + per-dealer breakdown via `nameOf`) client-side from whatever `trades` it's handed — for a dealer that's just their own trades (so the breakdown is trivially one row); for admin it's everyone's. The initial-load `useEffect` in `App` is role-branched: dealers fetch only `listTrades()` + `listStaff()` (for `nameOf`), never `listLeads()`/`listRMs()` — no point loading pipeline data a role can't see. Admin's top stat bar also shows a same-day total-brokerage figure (`todayBrokerage` in `App.tsx`) — see the stat-bar note below.

**Top stat bar**: `StatBar` always shows Total Leads / Active / Closed Won / Pipeline Value. There is no "Won Value" anymore (removed) — admin instead gets a 5th card, "Total Brokerage (Today)" (`todayBrokerage`, summed from `trades` filtered to today), via an optional `totalBrokerage` prop that's `undefined` for every other role (they never load `trades`, so showing a stat from empty data would be misleadingly "₹0" rather than absent).
